...
use crate::poly_iop::sum_check::{generic_sumcheck::SumcheckInstanceProof, batched_cubic_sumcheck::BatchedCubicSumcheckInstance};
...
pub struct BatchedRationalSumLayerProof<F: PrimeField> {
    ...
    pub left_claims: Vec<Fraction<F>>,
    pub right_claims: Vec<Fraction<F>>,
}
...
pub trait BatchedRationalSum<F: PrimeField>: Sized {
    ...
    fn verify_layers(
        ...
        for (layer_index, layer_proof) in proof_layers.iter().enumerate() {
            ...
            let lambda_layer = transcript.get_and_append_challenge(b"challenge_lambda_layer").unwrap();
            ...
            for (left, right) in layer_proof
                ...
                .zip(layer_proof.right_claims.iter())
            {
                transcript.append_field_element(b"sumcheck left claim p", &left.p).unwrap();
                transcript.append_field_element(b"sumcheck left claim q", &left.q).unwrap();
                transcript.append_field_element(b"sumcheck right claim p", &right.p).unwrap();
                transcript.append_field_element(b"sumcheck right claim q", &right.q).unwrap();
            }
...
pub trait BatchedRationalSumLayer<F: PrimeField>:
    BatchedCubicSumcheckInstance<F, Fraction<F>>
{
    ...
    fn prove_layer(
        ...
        // Random combination for p and q
        let lambda_layer: F = transcript.get_and_append_challenge(b"challenge_lambda_layer").unwrap();
        // produce a joint claim
        let claim = claims
            .iter()
            .zip(coeffs.iter())
        ...
        let mut eq_poly = DenseMultilinearExtension::from_evaluations_vec(r_rational_sum.len(), EqPolynomial::<F>::evals(r_rational_sum));
...
impl<F: PrimeField, const C: usize> BatchedCubicSumcheckInstance<F, Fraction<F>>
    for BatchedDenseRationalSumLayer<F, C>
{
    ...
    fn bind(&mut self, eq_poly: &mut DenseMultilinearExtension<F>, r: &F) {
        ...
        rayon::scope(|s| {
            ...
            s.spawn(|_| bind_poly_var_bot(eq_poly, r));
        });
        self.layer_len /= 2;
    }
    ...
    fn final_claims(&self) -> (Vec<Fraction<F>>, Vec<Fraction<F>>) {
...
impl<F: PrimeField, const C: usize> BatchedRationalSum<F> for BatchedDenseRationalSum<F, C> {
    ...
    fn claims(&self) -> Vec<Fraction<F>> {
        let num_layers =
            <BatchedDenseRationalSum<F, C> as BatchedRationalSum<F>>::num_layers(self);
    ...
    for BatchedSparseRationalSumLayer<F, C>
{
    ...
    fn compute_cubic(
        ...
        let eq_evals: Vec<(F, F, F)> = (0..eq_poly.evaluations.len() / 2)
            .map(|i| {
                let eval_point_0 = eq_poly[2 * i];
                let m_eq = eq_poly[2 * i + 1] - eq_poly[2 * i];
                let eval_point_2 = eq_poly[2 * i + 1] + m_eq;
                ...
                zip(layers, coeffs).for_each(|(layer, coeff)| match layer {
                    ...
                    DynamicDensityRationalSumLayer::Dense(dense_layer) => {
                        ...
                        let evals = eq_evals
                            ...
                            .fold(default_sum, |(sum_0, sum_2, sum_3), (a, b, c)| {
...
impl<F: PrimeField, const C: usize> BatchedRationalSum<F> for BatchedSparseRationalSum<F, C> {
    ...
    fn claims(&self) -> Vec<Fraction<F>> {
        ...
        left_claims
            .iter()
            .zip(right_claims.iter())
            .map(|(left_claim, right_claim)| Fraction::rational_add(*left_claim, *right_claim))
            .collect()
    }
...
mod rational_sum_tests {
    ...
    fn dense_prove_verify() {
        ...
        let claims =
            <BatchedDenseRationalSum<Fr, C> as BatchedRationalSum<Fr>>::claims(
                &batched_circuit,
            );
